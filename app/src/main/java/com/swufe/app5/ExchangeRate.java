...
//        更改使用Bunlder传递数据,使用标准的方式进行
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i(TAG, "onCreate: 不需要更新, 本次更新日期" + updateDate);

        if (!updateDate.equals(todayString)) {
            Log.i(TAG, "onCreate: 需要更新, 上次更新日期" + updateDate);
            Thread t = new Thread(ExchangeRate.this);
            t.start();
...
//        euroRate = 0.1256;
//        wonRate = 171.3421;
//        在主页面创建时调用SharedPreferences访问文件
//        文件为空,数值全部保存为0.0f
//      https://www.mrcjcn.com/n/56893.html 中国银行汇率怎么算
//        以100面额货币进行交易结算
...
//        第一次调用(或数值全0)则设置为原本的汇率设置
    ...
    public void onConfig(View view) {
        Intent intent = new Intent(ExchangeRate.this, RateSaveActivity.class);
        //使用Bundle一体化的传输数据
        Bundle bundle = new Bundle();
    ...
    public void onClick(View view) {
        ...
        if (str.equals("") || str.equals(R.string.message)) {
            Toast.makeText(ExchangeRate.this, "请输入金额", Toast.LENGTH_SHORT).show();
        } else {
